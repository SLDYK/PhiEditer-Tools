...
for i in range(0,len(cameradata)):
    ...
    if event[0]=="cp":
        ...
        campospoint[1].append(float(event[3]))
        ...
        campospoint[1].append(float(event[4]))
...
for j in track:
    ...
    for i in range(0,len(dealdata)):
        ...
        if int(event[1])==j and event[0]=="cp":
            ...
            trackpospoint[1].append(float(event[3]))
            ...
            trackpospoint[1].append(float(event[4]))
    ...
    for i in range(0,len(dealdata)):
        ...
        if int(event[1])==j and event[0]=="cm" and float(event[2])<=endtime and float(event[3])>=starttime:
            ...
            for m in range(0,len(timecut)-1):
                ...
                rotx=camstx-long*math.cos(newrot)+changex
